...
<html>
    <head>
        <meta charset="utf-8">
        <title>Commentaire Formation&Co</title>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST")
            {
                ...
                if (empty ($row['user_id'])) // on vérifie que l'utilisateur n'a pas déjà donné son avis
                {
                    ...
                    // use exec() because no results are returned
                        ...
                        $sql = "INSERT INTO `vote`( `user_id`, `acteur_id`, `votepositif`, `votenegatif`) 
                        VALUES ( :account_id, :acteur_id, :avispositif, :avisnegatif)";
                    
                    // use exec() because no results are returned
                        $statement=$conn->prepare($sql);
                        $statement->bindParam('account_id', $account_id);
                        $statement->bindParam('acteur_id', $acteur_id);
                        $statement->bindParam('avispositif', $avispositif);
                        $statement->bindParam('avisnegatif', $avisnegatif);
                        $statement->execute();

                        echo "<meta http-equiv='refresh' content='0;redirection_merci'>";
